...
namespace NCoreUtils.Logging
{
    public class AspNetCoreLogger : Logger
    {
        private ref struct HostingRequestFinishedLogWrapper
        {
            ...
            public void Apply(ref AspNetCoreContext context)
            {
        ...
        static private Func<string, Exception?, string> _passString = (s, _) => s;

        static private TService? GetServiceSafe<TService>(IServiceProvider? serviceProvider)
        ...
        private AspNetCoreContext GetCurrentAspNetCoreContext()
        {
            try
            {
                ...
                return loggingContext.AspNetCoreContext;
            }
            catch (Exception exn)
            {
        ...
        public override void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
        {
            ...
            if (state != null && state.GetType().FullName == "Microsoft.AspNetCore.Hosting.HostingRequestFinishedLog")
            {
                ...
                Provider.PushMessage(new AspNetCoreLogMessage<string>(CategoryName, logLevel, eventId, exception, evaluatedState, _passString, ctx, true));
            }
            else
            {
                evaluatedState = formatter(state, exception);
                Provider.PushMessage(new AspNetCoreLogMessage<string>(CategoryName, logLevel, eventId, exception, evaluatedState, _passString, GetCurrentAspNetCoreContext(), false));
            }
        }
    }
}