...
public class GoogleFluentdPayloadFactory(
    IGoogleFluentdSinkConfiguration configuration,
    IEnumerable<ILabelProvider> labelProviders)
    : GooglePayloadFactory<LogEntry, IGoogleFluentdSinkConfiguration>(configuration, labelProviders)
{
        ...
        bool isRequestSummary)
    {
        ...
        if (exception != null)
        {
            ...
            serviceContext = Configuration.Service is string { Length: > 0 } serviceName
                ? CreateOrUpdateServiceContext(
                    serviceName,
