...
public class FormattedStringPayloadFactory : IPayloadFactory<InMemoryByteSequence>
{
    ...
    private const int MaxPooledBufferSize = 32 * 1024; // 32K
    ...
    public InMemoryByteSequence CreatePayload<TState>(LogMessage<TState> message)
    {
        ...
        var payload = payloadSize <= MaxPooledBufferSize
            ? MemoryPool<byte>.Shared.Rent(payloadSize)
            : new UnmanagedMemoryManager<byte>(payloadSize);
